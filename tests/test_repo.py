"""Tests for homebrew-vectrade repository structure and documentation."""
...
class TestRepoStructure:
    ...
    def test_no_unexpected_formula_files(self):
        """Only vectrade.rb should be in Formula/."""
        formulas = list(FORMULA_DIR.glob("*.rb"))
        assert len(formulas) == 1
        assert formulas[0].name == "vectrade.rb"
...
class TestReadme:
    ...
    def test_has_h1_title(self, readme_content):
        assert readme_content.startswith("# homebrew-vectrade")
    ...
    def test_has_install_command(self, readme_content):
        assert "brew install vectrade" in readme_content

    def test_has_tap_command(self, readme_content):
        assert "brew tap VecTrade-io/vectrade" in readme_content
    ...
    def test_mentions_shell_completions(self, readme_content):
        assert "Completions" in readme_content or "completions" in readme_content
...
class TestCIWorkflow:
    ...
    def test_validates_ruby_syntax(self, ci_content):
        assert "ruby" in ci_content

    def test_runs_pytest(self, ci_content):
        assert "pytest" in ci_content

    def test_has_lint_job(self, ci_content):
        assert "lint" in ci_content.lower() or "Lint" in ci_content

    def test_has_test_job(self, ci_content):
        assert "test" in ci_content.lower()
